...
export default function Home({ auth, muestras }) {
  // Usamos useForm para manejar el formulario de visibilidad con onSuccess
  ...
  const handleVisibilityChange = (id, currentVisibility) => {
    ...
    patch(
      `/Admin/Muestras/${id}/update-visibility`,
      {
        visibilidad: newVisibility,
      },
      {
        onSuccess: () => {
          // Aquí actualizas la visibilidad en la interfaz (UI)
          const updatedMuestras = muestras.map((muestra) =>
            muestra.id === id ? { ...muestra, visibilidad: newVisibility } : muestra
          );
          setData('muestras', updatedMuestras);
            ...
            {muestras.map((muestra) => (
              <tr key={muestra.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-700">
                  ...
                  <label htmlFor={`visibilidad-${muestra.id}`} className="inline-flex items-center cursor-pointer">
                    ...
                    <input
                      ...
                      type="checkbox"
                      checked={muestra.visibilidad === 1} // Aseguramos que el checkbox esté marcado si visibilidad es 1
                ...
                <td className="px-4 py-3 flex space-x-2">
                  <Link
                    href={`/Admin/Muestras/${muestra.id}/edit`}
