...
export default function Home({ auth, muestras }) {
  ...
  const handleVisibilityChange = async (id, currentVisibility) => {
    // Convertir visibilidad a 0 o 1 en lugar de true o false
    const newVisibility = currentVisibility === 1 ? 0 : 1; // 1 = Visible, 0 = No visible

    try {
      // Enviar la solicitud PATCH para actualizar el campo visibilidad
      await patch(`/Admin/Muestras/${id}/update-visibility`, { visibilidad: newVisibility }, {
        preserveScroll: true, // Mantiene la posición en la página después de la actualización
      });

      // Aquí podrías agregar lógica para mostrar un mensaje de éxito si lo deseas
      console.log(`Visibilidad de la muestra ${id} actualizada a ${newVisibility === 1 ? 'Visible' : 'No visible'}`);
    } catch (error) {
                ...
                <td className="px-4 py-3 flex items-center space-x-2">
                  ...
                  <label htmlFor={`visibilidad-${muestra.id}`} className="inline-flex items-center cursor-pointer">
                    ...
                    <input
                      ...
                      type="checkbox"
                      checked={muestra.visibilidad === 1} // Aquí usamos 1 para visible
