...
const CreateSampleForm = ({ onClose, TypeTissues }) => {
    ...
    const handleChange = (e) => {
        ...
        if (id === 'image') {
            setData(id, files[0]);
        } else if (id === 'tissues_id') {
            // Buscar el objeto del tejido seleccionado
            const selectedTissue = TypeTissues.find(tissue => tissue.id === value);
            setData('tissues_id', selectedTissue?.id || '');
            setData('tissue_name', selectedTissue?.name || ''); // Almacenar el nombre
        } else {
            setData(id, value);
        }
    };
