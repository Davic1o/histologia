...
const OpenSeadragonViewer = () => {
  ...
  const initializeAnnotorious = (viewer) => {
    ...
    newAnno.on('createAnnotation', (annotation) => {
      console.log("Se ha creado una anotación:", annotation); // <-- Verifica si este mensaje aparece
      const { body, id, target } = annotation;

      // Actualiza el formulario con los datos de la anotación
      setData({
  ...
  const updateAnnotationList = () => {
    if (anno) {
      ...
      console.log("Anotaciones actuales en updateAnnotationList:", currentAnnotations); // <-- Verifica este log
